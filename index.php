<?php
$pass = "PHP-Proxy"; //默认密码，请自行修改
if (isset($_POST['password'])) {
    setcookie("password", $_POST['password'], time()+3600*24*366); //Cookie保存366天
    header("Refresh:0");
}
if ($_COOKIE['password'] != $pass) {
	header('HTTP/1.1 403');
    exit('<!DOCTYPE html><html><head><title>WoL\'s Proxy</title><meta charset="UTF-8"><link rel="stylesheet" type="text/css" href="//s0.pstatp.com/cdn/expire-1-M/bootswatch/3.4.0/paper/bootstrap.min.css"><style type="text/css">.row{margin-top:100px}.page-header{margin-bottom:90px}.expand-transition{margin-top:150px;-webkit-transition:all.5s ease;transition:all.5s ease}</style></head><body><div id="app" class="container"><div class="row row-xs"><div class="col-lg-6 col-md-6 col-sm-6 col-xs-10 col-xs-offset-1 col-sm-offset-3 col-md-offset-3 col-lg-offset-3"><div class="page-header"><h3 class="text-center h3-xs">WoL\'s Proxy</h3></div><meta charset="UTF-8"><label class="control-label" for="inputContent">请输入密码：</label><form method="post"><input type="password" class="form-control" name="password"/><input type="submit" class="input_group_addon btn btn-primary" value="GO"></form><div class="container"><p>©Powered by <a href="https://github.com/WoLeo-Z/PHP-Proxy">PHP-Proxy</a></p></div></footer></body></html>');
}
...
if (substr($path, 1, 7) == "http://" || substr($path, 1, 8) == "https://" || $_POST['urlss']) {
    ...
    exit('<html><head><meta charset="utf-8"><meta name="viewport" content="width=520, user-scalable=no, target-densitydpi=device-dpi"><title>WoL\'s Proxy</title><link rel="stylesheet" type="text/css" href="//s0.pstatp.com/cdn/expire-1-M/bootswatch/3.4.0/paper/bootstrap.min.css"><style type="text/css">.row{margin-top:100px}.page-header{margin-bottom:90px}.expand-transition{margin-top:150px;-webkit-transition:all.5s ease;transition:all.5s ease}</style></head><body><div id="app" class="container"><div class="row row-xs"><div class="col-lg-6 col-md-6 col-sm-6 col-xs-10 col-xs-offset-1 col-sm-offset-3 col-md-offset-3 col-lg-offset-3"><div class="page-header"><h3 class="text-center h3-xs">WoL\'s Proxy</h3></div><form method="post"><div class="form-group " id="input-wrap"><label class="control-label" for="inputContent">请输入需访问的链接：</label><input type="text" id="inputContent" class="form-control" name="urlss" placeholder="http://" required="required"></div><div class="text-right"><input type="submit" class="input_group_addon btn btn-primary" value="GO"></div></div></form></div></div><div align="center" class="expand-transition"><p>在当前链接末尾输入 ~q 可以退出当前页面回到首页</p><p>在域名后面加上链接地址即可访问，如 ' . $https . $host . '/https://google.com/ </p></div></div><footer class="footer navbar-fixed-bottom" style="text-align:center"><div class="container"><p>©Powered by <a href="https://github.com/WoLeo-Z/PHP-Proxy">PHP-Proxy</a></p></div></footer></body></html>');
}
//代理的域名及使用的协议最后不用加/
